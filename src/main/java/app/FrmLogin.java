...
public class FrmLogin extends JFrame {
	...
	void registrar() {
		...
		// validar los datos ingresados en la tabla -> Consulta
		EntityManagerFactory fabrica = Persistence.createEntityManagerFactory("jpa_sesion04");
		EntityManager em = fabrica.createEntityManager();		
		
		try {
			// select * from tb_usuarios where usr = ? and psw = ?
			Usuario u = em.createQuery(
			"select u from Usuario u where u.correo = :usr and u.clave = :psw", 
				Usuario.class).setParameter("usr", usuario).
							   setParameter("psw", clave).
							   getSingleResult();
